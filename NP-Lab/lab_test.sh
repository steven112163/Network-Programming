...
SESSION="lab_test"
SLEEP_TIME=0.5
if [ -z ${SERVER_TYPE} ] || [ -z ${SERVER_IP} ] || [ -z ${SERVER_PORT} ]; then
  echo "Usage: $0 <server type> <server ip> <server port>"
  exit 1
fi
...
for i in $(seq 0 1); do
  tmux send-keys -t ${i} "python3 ${SERVER_TYPE}client.py ${SERVER_IP} ${SERVER_PORT}" Enter
  sleep 0.5
done
